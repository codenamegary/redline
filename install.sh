...
verify_checksums() {
  ...
  (
    ...
    if [ -z "$expected" ]; then
      echo "checksums.txt has no entry for $tarball" >&2
      return 1
    fi
...
download_release() {
  ...
  curl -fsSL "$RELEASE_BASE/download/v$version/checksums.txt" -o "$tmp/checksums.txt" \
    || die "could not download checksums.txt for redline v$version"
